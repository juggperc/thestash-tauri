...
import { isTauri, saveFileToTauri as saveTauri, loadFileFromTauri as loadTauri, deleteFileFromTauri as deleteTauri, loadMetadataFromTauri as loadMetaTauri, saveMetadataToTauri as saveMetaTauri, getTauriFilePath as getPathTauri } from './tauri-storage'
...
export async function saveFileToElectron(fileId: string, file: File): Promise<boolean> {
  // Try Tauri first
  if (isTauri()) {
    return await saveTauri(fileId, file)
  }
...
export async function loadFileFromElectron(fileId: string, fileName: string): Promise<File | null> {
  // Try Tauri first
  if (isTauri()) {
    return await loadTauri(fileId, fileName)
  }
...
export async function deleteFileFromElectron(fileId: string, fileName: string): Promise<boolean> {
  // Try Tauri first
  if (isTauri()) {
    return await deleteTauri(fileId, fileName)
  }
...
export async function saveMetadataToElectron(metadata: { files: any[] }): Promise<boolean> {
  // Try Tauri first
  if (isTauri()) {
    return await saveMetaTauri(metadata)
  }
...
export async function getElectronFilePath(fileId: string, fileName: string): Promise<string | null> {
  // Try Tauri first
  if (isTauri()) {
    return await getPathTauri(fileId, fileName)
  }
